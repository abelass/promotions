...
function promotions_reservation_evenement_donnees_details($flux) {
	if (! _request('exec')) {
		...
		while ( $data = sql_fetch($sql) ) {
			...
			if ($details = charger_fonction('action', 'promotions/' . $data['type_promotion'], true) and ($data['date_debut'] == '0000-00-00 00:00:00' or (
									/*$data['date_debut']!='0000-00-00 00:00:00' AND */
									$data['date_debut'] <= $date)) and ($data['date_fin'] == '0000-00-00 00:00:00' or (
									/*$data['date_fin']!='0000-00-00 00:00:00' AND*/
									 $data['date_fin'] >= $date)) and ! in_array($id_evenement, $evenements_exclus_promotion) and (! $exclure_toutes or ($exclure_toutes and $exclure_toutes[0] == $id_promotion))) {
